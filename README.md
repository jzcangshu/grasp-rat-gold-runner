...
- 支持左上角“自动攻击”独立开关：未手动锁定时自动选择 150m 红圈内 HP 最低敌人，按目标运动轨迹、相对距离和子弹速度做短时预判。
- 自动攻击改为长按连发覆盖：每组一次性压住发射 5-8 发，并在远距离目标附近做覆盖偏移，提高一组子弹的命中覆盖率。
- HUD 左上侧提供 170m 攻击缓冲范围敌人列表，显示用户名、HP 和距离；点击可锁定自动攻击目标，目标仍在 500m 视野内时不会自动切换。
...
- 临时交战中自动躲避会同时调节与近身敌人的相对距离，优先保持在 100-150m；无弹体或弹体距离较远时会主动拉回距离带。
